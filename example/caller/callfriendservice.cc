...
int main(int argc, char **argv)
{
    ...
    stub.Getfriendlist(&controller, &request, &response, nullptr); // RpcChannel->RpcChannel::callMethod 集中来做所有rpc方法调用的参数序列化和网络发送  最后一个是done，但是传的是空，所以调用方不会使用回调函数done->Run()
